...
function createInputField() {
  ...
  stockNameInput.className =
    "w-2/5 px-3 py-2 border rounded-md mr-2 bg-gray-900";
  ...
  rupeeSymbol.className =
    "absolute inset-y-0 left-0 flex items-center pl-2 text-lg";
...
function addInputField(button) {
  ...
  var delButton = createButton(
    "Remove",
    function () {
      var index = stockEntries.findIndex(
        (entry) => entry.name === stockName && entry.price == price
      );
      if (index !== -1) {
...
function displayStockNames() {
  ...
  for (var j = 1; j <= x; j++) {
    ...
    roundEntry.innerHTML =
      '<label for="rounds" class="block text-white-700 font mb-2 ml-2">Round ' +
      j.toString() +
      " - </label";
